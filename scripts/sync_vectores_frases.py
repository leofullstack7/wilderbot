# scripts/sync_vectores_frases.py
import os
import math
from typing import List, Dict

from dotenv import load_dotenv
load_dotenv()

import firebase_admin
from firebase_admin import credentials, firestore

from openai import OpenAI
from pinecone import Pinecone, ServerlessSpec

# --- Config ---
OPENAI_MODEL_EMB = "text-embedding-3-small"   # 1536 dims, barato y bueno
EMBED_DIM = 1536
INDEX_NAME = os.getenv("PINECONE_INDEX", "wilder-frases")

# --- Init servicios ---
client = OpenAI(api_key=os.getenv("OPENAI_API_KEY"))

# Firebase Admin
cred = credentials.Certificate("credenciales/serviceAccountKey.json")
try:
    firebase_admin.get_app()
except ValueError:
    firebase_admin.initialize_app(cred)
db = firestore.client()

# Pinecone
pc = Pinecone(api_key=os.getenv("PINECONE_API_KEY"))

# Crear index si no existe
if INDEX_NAME not in [idx["name"] for idx in pc.list_indexes()]:
    print(f"Creando índice '{INDEX_NAME}' en Pinecone...")
    pc.create_index(
        name=INDEX_NAME,
        dimension=EMBED_DIM,
        metric="cosine",
        spec=ServerlessSpec(cloud="aws", region="us-east-1"),
    )
index = pc.Index(INDEX_NAME)

def chunk_text(text: str, max_tokens: int = 500) -> List[str]:
    """
    Sencillo chunker por longitud (caracteres).
    Para frases cortas no hace falta chunking, pero lo dejamos listo.
    """
    # ~4 chars ≈ 1 token (muy aproximado)
    max_chars = max_tokens * 4
    if len(text) <= max_chars:
        return [text]
    chunks = []
    start = 0
    while start < len(text):
        end = min(start + max_chars, len(text))
        # intenta cortar en final de oración
        cut = text.rfind(".", start, end)
        if cut == -1 or cut - start < max_chars * 0.5:
            cut = end
        chunks.append(text[start:cut].strip())
        start = cut
    return [c for c in chunks if c]

def embed_texts(texts: List[str]) -> List[List[float]]:
    resp = client.embeddings.create(model=OPENAI_MODEL_EMB, input=texts)
    return [d.embedding for d in resp.data]

def sync_frases_wilder():
    print("Descargando frases desde Firestore...")
    docs = db.collection("frases_wilder").stream()

    upserts = []
    count = 0

    for doc in docs:
        data: Dict = doc.to_dict() or {}
        frase_id = doc.id  # <- ya usas IDs numéricos en Firestore
        texto = data.get("texto", "").strip()
        if not texto:
            continue

        # Chunk opcional (para frases suele ser 1)
        chunks = chunk_text(texto, max_tokens=400)

        # Embeddings
        vectors = embed_texts(chunks)

        # Construye items para Pinecone
        for i, (chunk, emb) in enumerate(zip(chunks, vectors)):
            vector_id = f"{frase_id}-{i}"  # único por chunk
            metadata = {
                "frase_id": frase_id,
                "texto": chunk,
                "tono": data.get("tono", ""),
                "contexto": data.get("contexto", ""),
                "palabras_clave": data.get("palabras_clave", []),
                "tipo": "frase_wilder",
            }
            upserts.append({"id": vector_id, "values": emb, "metadata": metadata})
            count += 1

        # Subir por lotes de 100
        if len(upserts) >= 100:
            print(f"Upserting {len(upserts)} vectores...")
            index.upsert(vectors=upserts)
            upserts = []

    # Último batch
    if upserts:
        print(f"Upserting {len(upserts)} vectores (final)...")
        index.upsert(vectors=upserts)

    print(f"✅ Sincronización completa. {count} vectores indexados/actualizados en '{INDEX_NAME}'.")

if __name__ == "__main__":
    sync_frases_wilder()
